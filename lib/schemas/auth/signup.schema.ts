...
export const adminUsernameRegex = /^[a-zA-Z0-9]+$/;
export const koreanUsernameRegex = /^[가-힣ㄱ-ㅎㅏ-ㅣ]+$/;
...
export const AdminSignUpSchema = createInsertSchema(users)
  ...
  .extend({
    region: z.string({ required_error: "권역은 필수 선택 항목입니다." }).trim(),
    email: z
...
export const UserSignUpSchema = createInsertSchema(users)
  ...
  .extend({
    email: z
      ...
      .trim(),
    company: z
      .string({ required_error: "회사명은 필수 입력 항목입니다." })
      .trim(),
    companyPhone: z
